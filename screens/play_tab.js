...
const Play_tab = ({route, navigation}) => {
  ...
  const myCustomShare = async () => {
    const shareOptions = {
      message:
        'nocap is a new way to get trusted and verified information from the public.\n\nDownload nocap from the link below and get started.\n\n' +
        url,
      url: `${url}/${user}`,
  ...
  const send_msg = async () => {
    try {
      const response = await fetch(
        'https://1559-2409-4055-202-875b-81-c377-293d-1245.ngrok.io/alert',
        {
          method: 'post',
          headers: {
    ...
    <View style={{flex: 1}}>
      <View style={{alignItems: 'center'}}>
        ...
        <View
          ...
          <Text
            onPress={() =>
              Linking.openURL(`nocapp.link/${user}`)
            }>{`nocapp.link/${user}`}</Text>
        </View>
        <LinearGradient
          ...
          <Text
            ...
            selectable={true}
            onPress={() => copyToClipboard(`nocapp.link/${user}`)}>
            Copy link
          </Text>
        </LinearGradient>
        ...
        <View style={{width: width * 0.92, marginTop: 30}}>
          ...
          <View
            ...
            <View
              ...
              <Text
                style={{marginLeft: 15, width: width * 0.8}}>
